...
export function TrackingStatus({ profile }: TrackingStatusProps) {
  ...
  return (
    <section className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-lg border border-zinc-700 bg-zinc-800/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className={`h-2.5 w-2.5 rounded-full ${agentActive ? 'bg-emerald-500' : 'bg-zinc-600'}`} />
          </div>

          {!hasAgent && (
            <a
              href="/download"
              className="mt-3 block text-center rounded-md bg-zinc-700 px-3 py-1.5 text-xs font-medium text-zinc-300 transition-colors hover:bg-zinc-600"
            >
              Download Agent
            </a>
          )}
        ...
        <div className="rounded-lg border border-zinc-700 bg-zinc-800/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className={`h-2.5 w-2.5 rounded-full ${hasDiscord ? 'bg-emerald-500' : 'bg-zinc-600'}`} />
          </div>
      ...
      {/* Priority Info */}
      {hasAgent && hasDiscord && (
        <div className="mt-4 rounded-lg border border-zinc-700 bg-zinc-800/30 px-4 py-3">
