...
async function tryReopenSession(
  ...
  if (discordStart.getTime() <= lastStartedAt.getTime()) {
    ...
    if (reopenError) {
      logger.error('[DB] Failed to reopen session', reopenError, {
        userId,
    ...
    logger.info('[DB] Session reopened (same game launch)', {
      userId,
      gameName,
      sessionId: lastSession.id,
    });
...
export async function createSession(
  ...
  try {
    ...
    logger.error('[DB] Game resolution failed', error, { userId, gameName: game.name });
  }
  ...
  if (error) {
    logger.error('[DB] Failed to create session', error, { userId, gameName: game.name });
    return null;
  }
...
export async function closeSession(userId: string): Promise<boolean> {
  ...
  if (activeSession.source !== 'discord') return false;
  ...
  if (error) {
    logger.error('[DB] Failed to close session', error, {
      userId,
...
export async function touchActiveSessions(sessionIds: string[]): Promise<void> {
  ...
  if (error) {
    logger.error('[DB] Failed to touch active sessions', error, { sessionIds });
  }
}
